...
def main(args):
    ...
    if args.task in ["slcp_summary", "slcp2_summary", "slcp3_summary", "slcp3"]:
    ...
    L_pilot = int(10_000 / args.tol_pilot)
    Y_cal = priors.sample((L_pilot,))
    ...
    if args.task in ["slcp_summary", "slcp2_summary", "slcp3_summary", "slcp3", "bernoulli_glm"]:
    ...
    elif args.task in ["MoG_10", "Lapl_10"]:
    ...
    with torch.no_grad():
        _, adj = calibrate_cov(s_dp_tmp, X_cal, Y_cal, mean_net, covnet, tol=args.tol_pilot, device = device, case = args.task, chunk_size=chunk_size_cal, bounds = bounds)
    ...
    if args.task in ["slcp", "slcp_summary"]:
        ...
        indices = torch.randperm(num_rows)[:10000]
        post_sample = torch.tensor(post_sample[indices,:], dtype = torch.float32)
    elif args.task in ["bernoulli_glm", "MoG_5", "Lapl_5", "MoG_10", "Lapl_10"]:    
    ...
    sci_str2 = format(args.tol_pilot, ".0e")
    

    output_dir = f"./NABC_results/pilot_experiment/{args.task}_withcov_exp/mean_{int(num_training_mean/1_000)}K_cov_{int(num_training_cov/1_000)}K_resample/amor_{int(args.L/1_000_000)}M_eta{sci_str}_pilot{sci_str2}"
...
def get_args():
    ...
    parser.add_argument("--tol_pilot", type=float, default=1e-2,
                    help="Tolerance value for pilot NABC (any positive float < 1")
...
if __name__ == "__main__":
    ...
    print(f"tol_pilot: {args.tol_pilot}")