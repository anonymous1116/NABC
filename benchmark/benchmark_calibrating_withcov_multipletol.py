...
def main(args):
    ...
    with torch.no_grad():
        _, adj = calibrate_cov(s_dp_tmp, X_cal, Y_cal, mean_net, covnet, tol=.1, device = device, case = args.task, chunk_size=chunk_size_cal, bounds = bounds)
    ...
    if args.tol == 1e-6:
        tols = [1, 5e-1, 1e-1, 5e-2, 1e-2, 5e-3, 1e-3]
    elif args.tol == 1e-5:
        tols = [1, 6e-1, 3e-1, 1e-1, 6e-2, 3e-2, 1e-2]

    for j in range(len(tols)):
        NABC_results = []
        start_time = time.time()
        calibrate_results = calibrate_cov(s_dp_tmp, X_abc, Y_abc, mean_net, covnet, n_samples = 10000, tol = tols[j], bounds = bounds)
        end_time = time.time()
        elpased_time2 = end_time - start_time
        ...
        torch.save([torch.cuda.get_device_name(0), elapsed_time+ elpased_time2], f"{output_dir}/x0{args.x0_ind}_seed{args.seed}_info.pt")
...
def get_args():
    ...
    parser.add_argument("--seed", type = int, default = 1,
                        help = "See number (default: 1)")
    parser.add_argument("--L", type = int, default = 10_000_000_000,
                        help = "Calibration data size (default: 10B)")
