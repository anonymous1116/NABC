...
def NDP_train_weight(X, Y, net_str, device="cpu", p_train=0.7, N_EPOCHS=250, lr=1e-3, val_batch = 10_000, early_stop_patience = 20):
    torch.set_default_device(device)
    X = X.to(device)
    Y = Y.to(device)
    net = copy.deepcopy(net_str)
    ...
    del net, X_train, Y_train, X_val, Y_val, dataloader, weight_1
    print(f"============= Best validation loss: {best_val_loss} =============", flush = True)
    return best_model_state, val_error_plt, best_val_loss


def NDP_train_l2(X, Y, net_str, device="cpu", p_train=0.7, N_EPOCHS=250, lr=1e-3, val_batch = 10_000, early_stop_patience = 20):
    torch.set_default_device(device)
    X = X.to(device)
    Y = Y.to(device)
    net = copy.deepcopy(net_str)
    ...
    for epoch in range(N_EPOCHS):
        ...
        with torch.no_grad():
            ...
            for (x_batch, y_batch) in eval_val_dataloader:
                y_pred_batch = net(x_batch)
                batch_loss = mse_loss(y_pred_batch, y_batch).item()
                val_loss_accum += batch_loss
            val_loss = val_loss_accum * val_batch/ L_val
            val_error_plt.append(torch.tensor(val_loss))
    ...
    torch.cuda.empty_cache()
    del net, X_train, Y_train, X_val, Y_val, dataloader
    print(f"============= Best validation loss: {best_val_loss} =============", flush = True)
    return best_model_state, val_error_plt, best_model_state



def cond_mad_train(X, resid, net_var, device = "cpu", p_train = 0.7, N_EPOCHS = 250, lr = 1e-3, val_batch = 10_000, early_stop_patience = 50):
    torch.set_default_device(device)
    X = X.to(device)
    resid = resid.to(device)
    resid = torch.max(torch.abs(resid), torch.ones(1) * 1e-30).log()
    
    return NDP_train(X, resid, net_var, device, p_train, N_EPOCHS, lr, val_batch = val_batch, early_stop_patience = early_stop_patience)



def calibrate(x0, X_cal, y_cal, net, net_var, n_samples= 10000, tol = .01, bounds = None, device = "cpu", case = None, chunk_size = 10_000):
    if case in ["slcp"]:
        x0_tmp = SLCP_summary(x0)
        X_cal_tmp = SLCP_summary(X_cal)
        x0 = x0.to(device)
        X_cal = X_cal.to(device)
        mad = compute_mad(X_cal)
        mad = torch.reshape(mad, (1, X_cal.size(1))).to(device)
        dist = torch.sqrt(torch.mean(torch.abs(X_cal_tmp.to(device) - x0_tmp.to(device))**2, 1))
    else:
        x0 = x0.to(device)
        X_cal = X_cal.to(device)
        mad = compute_mad(X_cal)
    ...
    #    dist = torch.sqrt(torch.mean(torch.abs(X_cal - x0)**2, 1))
...
def calibrate_cov(x0, X_cal, y_cal, mean_net, covnet, n_samples= 10000, tol = .01, bounds = None, device = "cpu", case = None, chunk_size = 10_000):
    if case in ["slcp", "slcp3"]:
        x0_tmp = SLCP_summary_transform2(x0)
        X_cal_tmp = SLCP_summary_transform2(X_cal)
        x0_tmp = x0_tmp.to(device)
        X_cal_tmp = X_cal_tmp.to(device)
        mad = compute_mad(X_cal_tmp)
        mad = torch.reshape(mad, (1, X_cal_tmp.size(1))).to(device)
        dist = torch.sqrt(torch.mean(torch.abs(X_cal_tmp.to(device) - x0_tmp.to(device))**2/mad**2, 1))
    elif case in ["OU"]:
        dist = Markov_sliced_wasserstein_distance(X_cal,x0, chunk_size=10_000)
    else:
        x0 = x0.to(device)
        X_cal = X_cal.to(device)
        mad = compute_mad(X_cal)
...
def calibrate_cov_transform(x0, X_cal, y_cal, mean_net, covnet, n_samples= 10000, tol = .01, bounds = None, transform = None, device = "cpu", case = None, chunk_size = 10_000):
    if transform is None:
        raise ValueError("calibrate_cov_transform: `transform` must be provided")
    ...
    if case in ["slcp", "slcp3"]:
        ...
        dist = Markov_sliced_wasserstein_distance(X_cal, x0)
    else:
        x0 = x0.to(device)
        X_cal = X_cal.to(device)
        mad = compute_mad(X_cal)
    ...
    mean_net = mean_net.to(device)
    y_cal = torch.clone(y_cal[wt1,:])
    y_cal_transform = transform.forward(y_cal)
    ...
    with torch.no_grad():
        for i in range(num_chunks + 1):
            ...
            X_chunk = X_cal[start:end].to(device)
            y_chunk = y_cal_transform[start:end].to(device)
            y_chunk_predict = mean_net(X_chunk)
    ...
    vec = adj.numpy()
    sam_ind = np.random.choice(np.arange(0, adj.size(0)), adj.size(0), replace = False, p = P)
    sample_post_large = torch.tensor(vec[sam_ind,:])
    sample_post_large = transform.inverse(sample_post_large)
    del P, sam_ind
...
def calibrate_cov3_transform(x0, X_cal, y_cal, mean_net, covnet, n_samples= 10000, tol = .01, bounds = None, transform = None, device = "cpu", case = None, chunk_size = 10_000):
    if transform is None:
        raise ValueError("calibrate_cov_transform: `transform` must be provided")
    ...
    with torch.no_grad():
        mean_net.eval()
        thetahat = mean_net(x0)
        thetahat = torch.clone(transform.inverse(thetahat))
    ...
    with torch.no_grad():
        for i in range(num_chunks + 1):
            ...
            y_chunk_predict = mean_net(X_chunk)
            y_chunk_predict = torch.clone(transform.inverse(y_chunk_predict.to("cpu")))

            #if bounds is not None:
            #    bounds_tensor = torch.tensor(bounds).to(device)
            #    y_chunk_predict = torch.clamp(y_chunk_predict, bounds_tensor[:,0] ,bounds_tensor[:,1]) 
            resid_chunk = y_chunk - y_chunk_predict.to(device)
...
def calibrate_cov_ds(x0, X_cal, y_cal, mean_net, covnet, n_samples= 10000, tol = .01, bounds = None, device = "cpu", case = None, chunk_size = 10_000):
    if case in ["slcp", "slcp3"]:
        x0_tmp = SLCP_summary_transform2(x0.reshape(-1,8))
        X_cal_tmp = SLCP_summary_transform2(X_cal.reshape(-1,8))
        x0_tmp = x0_tmp.to(device)
        X_cal_tmp = X_cal_tmp.to(device)
        mad = compute_mad(X_cal_tmp)
        mad = torch.reshape(mad, (1, X_cal_tmp.size(1))).to(device)
