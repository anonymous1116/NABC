...
def main(args):
    ...
    print(X.size(), Y.size())
    num_training_mean = 500_000
    net_dir = f"../depot_hyun/hyun/NDP/bernoulli_glm/train_{int(num_training_mean/1_000)}K/bernoulli_glm{args.seed}_mean.pt"
    ...
    for epoch in range(N_EPOCHS):
        ...
        if epochs_no_improve >= early_stop_patience:
            ...
            early_stop = True
            print(f"Best: {best_val_loss}",flush = True)
            break
    ...
    output_dir = f"../depot_hyun/hyun/NDP/{args.task}/cov2_{int(args.num_training/1_000)}K_layer_{args.layer_len}"
...
if __name__ == "__main__":
    ...
    print(f"task: {args.seed}")
    print(f"task: {args.task}")
    print(f"task: {args.num_training}")
    print(f"task: {args.layer_len}")
    print(f"task: {args.N_EPOCHS}")