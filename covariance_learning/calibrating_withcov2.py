...
def main(args):
    ...
    net_dir = f"../depot_hyun/hyun/NDP/{args.task}/train_500K/bernoulli_glm{seed}_mean.pt"
    tmp1 = torch.load(net_dir)

    net_dir = f"../depot_hyun/hyun/NDP/{args.task}/cov2_{int(num_training/1_000)}K_layer_{args.layer_len}"
    ...
    output_dir = f"./NABC_results/{args.task}_withcov2/{int(num_training/1_000)}K_resample/amor_{int(args.L/1_000_000)}M_eta{sci_str}"
