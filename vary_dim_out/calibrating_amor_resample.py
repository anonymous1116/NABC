...
def main(args):
    ...
    for dim_out in range(1, 11):
        ...
        X_cal, Y_cal = synthetic_fn(1_000_000, dim_out)
        
        if task == "MoG":
            simulator = MoG
        else:
            raise ValueError(f"Unsupported simulator: {task}")

        chunk_size_cal = 10_000
        with torch.no_grad():
            _, adj = calibrate(s_dp_tmp, X_cal, Y_cal, net, net2, tol=1, bounds=bounds, device = device, case = args.task, chunk_size=chunk_size_cal)
    ...
    output_dir = f"./NABC_results/{task}/{int(num_training/1_000)}K_resample/amor_{int(args.L/1_000_000)}M_eta{sci_str}"
