...
from NDP import NDP_train, calibrate, learning_checking
from NDP_functions import s_dp_Lapl, synthetic_pairs_Lapl, ABC_rej, true_samples_Lapl
...
def main(args):
    ...
    for dim_out in range(1, 11):
        ...
        net = FL_Net_bounded(D_in, D_out, H=Hs, p = 0.1, bounds = [[-5,5]]*dim_out)
        ...
        for i in range(num_chunks + 1):
            ...
            if nums == 0:
                break
            X_chunk, Y_chunk =  synthetic_pairs_Lapl(nums, dim_out)
        ...
        post_sample  = true_samples_Lapl(dim_out)
        s_dp_tmp = s_dp_Lapl(dim_out)
    ...
    output_dir = f"./NABC_results/{int(args.L/1_000_000)}M_eta{sci_str}"
    ...
    torch.save(NABC_results, f"{output_dir}/seed{args.seed}.pt")
