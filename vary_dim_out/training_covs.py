...
from module import CovarianceNet, FL_Net, FL_Net_bounded
...
def main(args):
    ...
    X, Y =  synthetic_fn(args.num_training_mean, args.dim_out)


    print(X.size(), Y.size())
    net_dir = f"../depot_hyun/hyun/NDP/{args.task}/mean_{int(args.num_training_mean/1_000)}K/dim_out_{args.dim_out}/{args.task}{args.seed}_mean.pt"
    
    tmp = torch.load(net_dir)

    bounds_base = [-5,5] if args.task == "Lapl" else [-10,10]  
    bounds = [bounds_base]*args.dim_out
    ...
    output_dir = f"../depot_hyun/hyun/NDP/{args.task}/mean_{int(args.num_training_mean/1000)}K_cov_{int(args.num_training_cov/1_000)}K_layer_{args.layer_len}/dim_out_{args.dim_out}"
