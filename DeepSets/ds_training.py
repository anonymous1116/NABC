...
from ds_module import DeepSets2D, DeepSets2D_bounded
import time
...
def main(args):
    ...
    output_dir = f"../depot_hyun/hyun/NDP/DS/{args.task}/train_{int(args.num_training/1_000)}K_{args.layer_len}/"
    ...
    if bounds is None:
        net = DeepSets2D(input_dim = 2, hidden = args.layer_len, out_dim = D_out).to(device)
    else:
        net = DeepSets2D_bounded(input_dim = 2, hidden = args.layer_len, out_dim = D_out, bounds = bounds).to(device)
...
python ./DeepSets/ds_cov_training.py \
