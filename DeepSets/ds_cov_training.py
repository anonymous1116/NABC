...
from ds_module import DeepSets_CovarianceNet, DeepSets2D, DeepSets2D_bounded
...
def main(args):
    ...
    #X = X.reshape(-1,4,2)
    print(X.size(), Y.size())
    net_dir = f"../depot_hyun/hyun/NDP/DS/{args.task}/train_{int(args.num_training_mean/1_000)}K_{int(args.layer_len)}/{args.task}{args.seed}_mean.pt"
    ...
    # Model 
    covnet = DeepSets_CovarianceNet(input_dim = 2, hidden_dim = args.layer_len, output_dim = D_out)
    ...
    output_dir = f"../depot_hyun/hyun/NDP/DS/{args.task}/mean_{int(args.num_training_mean/1000)}K_cov_{int(args.num_training_cov/1_000)}K_layer_{args.layer_len}"
