...
from batch_process import resid_chunk_process
from module import GRU_net, GRU_net_bounded
import time
...
def main(args):
    ...
    output_dir = f"../depot_hyun/hyun/NDP/{args.task}/train_{int(args.num_training/1_000)}K_{int(args.layer_len)}/"
    ...
    if bounds is None:
        net = GRU_net(input_dim = 1, hidden_dim = Hs, output_dim = D_out).to(device)
    else:
        net = GRU_net_bounded(input_dim = 1, hidden_dim = Hs, output_dim = D_out, bounds = bounds).to(device)
    ...
    net.eval()
    resid = resid_chunk_process(X_train2, Y_train2, net, device = device, bounds = bounds, chunk_size = 1_000)

    resid = torch.max(torch.abs(resid), torch.ones(1) * 1e-30).log()
    net2 = GRU_net(input_dim = 1, hidden_dim = Hs, output_dim = D_out) 
...
python ./SDE/SDE_cov_training.py \
