...
import sys
import subprocess
from sensitivity_nets import FL_Net_bounded_sen, FL_Net_sen2
...
def main(args):
    ...
    output_dir = f"../depot_hyun/hyun/NDP/sensitivity3/{args.task}/J{int(args.num_training/1_000)}K_{args.layer_len}_{args.layer_num}"
    ...
    if bounds is None:
        net = FL_Net_sen2(D_in = D_in, D_out = D_out, H = Hs, p = 0.00, num = args.layer_num)
    else:
        net = FL_Net_bounded_sen(D_in = D_in, D_out = D_out, H = Hs, p = 0.00, bounds = bounds, num = args.layer_num)
...
"""
    ...
    output_dir = f"../NDP/benchmark/cov_learning/sensitivity3/{args.task}/J_{int(args.num_training/1000)}K/slurm_files"
    os.makedirs(output_dir, exist_ok=True)
