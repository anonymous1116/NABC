...
def main(args):
    ...
    if args.task in ["slcp_summary", "slcp2_summary", "slcp3_summary", "slcp3", "my_slcp2", "my_slcp3"]:
    ...
    if args.task == "slcp2_summary":
        net_dir = f"../depot_hyun/hyun/NDP/slcp_experiment/slcp_summary/mean_{int(num_training_mean/1000)}K_cov3_{int(num_training_cov/1_000)}K_layer_{args.layer_len}"
    elif args.task == "my_slcp2":
        net_dir = f"../depot_hyun/hyun/NDP/slcp_experiment/my_slcp/mean_{int(num_training_mean/1000)}K_cov3_{int(num_training_cov/1_000)}K_layer_{args.layer_len}"
    else:
        net_dir = f"../depot_hyun/hyun/NDP/slcp_experiment/{args.task}/mean_{int(num_training_mean/1000)}K_cov3_{int(num_training_cov/1_000)}K_layer_{args.layer_len}"
    ...
    if args.task in ["slcp_summary", "slcp2_summary", "slcp3_summary", "slcp3", "bernoulli_glm", "my_slcp2", "my_slcp3"]:
    ...
    output_dir = f"./NABC_results/slcp_experiment/{args.task}_withcov/mean_{int(num_training_mean/1_000)}K_cov3_{int(num_training_cov/1_000)}K_resample/amor_{int(args.L/1_000_000)}M_eta{sci_str}"
