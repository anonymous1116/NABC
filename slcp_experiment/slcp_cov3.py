...
def main(args):
    ...
    print(X.size(), Y.size())
    net_dir = f"../depot_hyun/hyun/NDP/slcp_experiment/{args.task}/train_{int(args.num_training_mean/1_000)}K/{args.task}{args.seed}_mean.pt"
    ...
    #Y = torch.clone(transform.forward(Y))
    ...
    for epoch in range(N_EPOCHS):
        ...
        for id_batch, (x_batch, y_batch) in enumerate(dataloader):
            ...
            mu_batch = torch.clone(transform.inverse(mu_batch))
    ...
    output_dir = f"../depot_hyun/hyun/NDP/slcp_experiment/{args.task}/mean_{int(args.num_training_mean/1000)}K_cov3_{int(args.num_training_cov/1_000)}K_layer_{args.layer_len}"
