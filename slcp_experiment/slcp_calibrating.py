...
from NDP import calibrate_cov_transform
...
def main(args):
    ...
    if args.task == "slcp2_summary":
        net_dir = f"../depot_hyun/hyun/NDP/slcp_experiment/slcp_summary/mean_{int(num_training_mean/1000)}K_cov_{int(num_training_cov/1_000)}K_layer_{args.layer_len}"
    elif args.task == "my_slcp2":
        net_dir = f"../depot_hyun/hyun/NDP/slcp_experiment/my_slcp/mean_{int(num_training_mean/1000)}K_cov_{int(num_training_cov/1_000)}K_layer_{args.layer_len}"
    else:
        net_dir = f"../depot_hyun/hyun/NDP/slcp_experiment/{args.task}/mean_{int(num_training_mean/1000)}K_cov_{int(num_training_cov/1_000)}K_layer_{args.layer_len}"
    ...
    with torch.no_grad():
        _, adj = calibrate_cov_transform(s_dp_tmp, X_cal, Y_cal, mean_net, covnet, transform = transform, tol=.01, device = device, case = args.task, chunk_size=chunk_size_cal, bounds = bounds)
    ...
    calibrate_results = calibrate_cov_transform(s_dp_tmp, X_abc, Y_abc, mean_net, covnet, transform = transform, n_samples = 10000, tol = tol, bounds = bounds)
    end_time = time.time()
    ...
    output_dir = f"./NABC_results/slcp_experiment/{args.task}_withcov/mean_{int(num_training_mean/1_000)}K_cov_{int(num_training_cov/1_000)}K_resample/amor_{int(args.L/1_000_000)}M_eta{sci_str}"
